...
LLM-GNN combine Graph Neural Networks (GNN) et Large Language Models (LLM) pour analyser les relations et similarités entre villes.
Le projet utilise un graphe construit à partir de OpenStreetMap (OSM) pour prédire les liens entre villes et générer des explications textuelles sur la similarité des villes selon population, localisation et pays.
...
Le graphe contient 3363 nœuds et 13547 arêtes.

Chaque nœud représente une ville avec : longitude, latitude, population, pays et nom de la ville.

Les colonnes numériques (lon, lat, population) sont utilisées pour les features GNN.

Les colonnes textuelles (country, city_name) servent uniquement à générer des explications via le LLM.
...
💡 Installation exemple via conda
